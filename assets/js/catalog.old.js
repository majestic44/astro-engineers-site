      ...
      ${(() => {
        ...
        const bpBox = bp ? `
          <div class="priceBox">
            ...
            <div class="value">${escapeHtml(bp)}</div>
          </div>` : "";

        const tkBox = tk ? `
          <div class="priceBox">
            <div class="label">Token Only</div>
            <div class="value">${escapeHtml(tk)}</div>
          </div>` : "";

        const boxes = (bpBox + tkBox).trim();
        if(!boxes) return "";
        return `<div class="priceRow">${bpBox}${tkBox}</div>`;
      })()}
